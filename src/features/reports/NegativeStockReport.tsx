...
import { Search, AlertCircle, Download, Printer, ArrowUpDown, FlaskConical } from 'lucide-react';
import { StockBalance } from '@/types';

export default function NegativeStockReport() {
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden flex flex-col mb-8">
      ...
      <div className="p-6 border-b border-gray-100 flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 bg-gray-50/50">
        <div>
          <h1 className="text-2xl font-extrabold text-red-900 tracking-tight">รายงานยอดคงคลังติดลบ</h1>
          <p className="text-sm text-red-600 font-medium mt-1">แสดงรายการเวชภัณฑ์ที่มียอดคงคลังในระบบน้อยกว่า 0 (รอรับเข้า หรือมีการตัดจ่ายเกิน)</p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row items-center gap-3 w-full lg:w-auto no-print">
          <div className="relative w-full sm:w-72">
            ...
            <input
              type="text"
              placeholder="ค้นหาชื่อเวชภัณฑ์, Abbreviation, Lot..."
              value={search}
          ...
          <div className="flex gap-2 w-full sm:w-auto">
            <button
              onClick={() => navigate('/reports/negative-stock/analysis')}
              className="flex-1 sm:flex-initial flex items-center justify-center gap-2 px-4 py-2.5 bg-amber-500 hover:bg-amber-600 text-white rounded-xl transition-all shadow-sm text-sm font-bold cursor-pointer"
              title="วิเคราะห์สาเหตุยอดติดลบ (Dev)"
            >
              <FlaskConical size={16} />
              <span>วิเคราะห์สาเหตุ (Dev)</span>
            </button>
            <button
            ...
            <button
              ...
              className="flex-1 sm:flex-initial flex items-center justify-center gap-2 px-4 py-2.5 bg-red-600 hover:bg-red-700 text-white rounded-xl transition-all shadow-md text-sm font-bold cursor-pointer"
              title="พิมพ์รายงานสต๊อกติดลบ (PDF)"
            >
              <Printer size={16} />
