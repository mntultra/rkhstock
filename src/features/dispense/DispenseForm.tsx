...
function CustomLotSelect({ value, onChange, options, hasError, setCellRef, onKeyDown, unitName }: {
  ...
  return (
    <div ref={ref} className="relative w-full">
      ...
      {open && (
        <div className="absolute z-[60] min-w-[320px] left-0 mt-2 bg-white border border-gray-100 rounded-2xl shadow-xl overflow-hidden max-h-72 flex flex-col">
          {activeLots.length > 0 && (
  ...
  return (
    <div className="max-w-full mx-auto space-y-6 pb-20">
      ...
      <Card className="overflow-visible animate-fade-in-up">
        ...
        <div className="min-h-[250px] overflow-visible pb-32">
          <table className="w-full text-left border-collapse text-sm">
            <thead>
              <tr className="border-b border-gray-100 text-gray-400 font-extrabold text-xs uppercase tracking-wider bg-gray-50/50">
                ...
                <th className="py-3.5 px-4 min-w-[350px]">เวชภัณฑ์ (ค้นหา / สแกน)</th>
                <th className="py-3.5 px-4 w-32 text-center">จำนวนจ่าย</th>
                <th className="py-3.5 px-4 w-56 text-center">ล็อต (FEFO)</th>
                <th className="py-3.5 px-4 w-32 text-center">วันหมดอายุ</th>
                <th className="py-3.5 px-4 w-40">หมายเหตุ</th>
            ...
            <tbody className="divide-y divide-gray-50 overflow-visible">
              {rows.map((row, index) => {
                ...
                return (
                  <Fragment key={row.id}>
                    <tr className="hover:bg-gray-50/30 transition-colors group overflow-visible relative" style={{ zIndex: 100 - index }}>
                        ...
                        {!row.product || editingRowId === row.id ? (
                          <div className="relative z-50 flex gap-1.5 items-center">
                            <div className="flex-1 min-w-0">
                              <ProductSearchInput
                                ...
                                placeholder={row.product ? "ค้นหายาเพื่อเปลี่ยน..." : "พิมพ์ค้นหายา หรือรหัสคีย์เพื่อเริ่ม..."}
                          ...
                          <div className="flex items-center gap-1.5">
                            <div 
                              ...
                              className="flex-1 flex items-center justify-between p-2.5 bg-emerald-50/60 hover:bg-emerald-100/60 border border-emerald-100 hover:border-emerald-300 rounded-xl text-emerald-900 shadow-inner cursor-pointer transition-all group/item min-w-0"
                            ...
                            >
                              <div className="flex flex-col gap-1 w-full min-w-0">
                                ...
                                <div className="flex items-center flex-wrap gap-1.5 mt-0.5">
                                  <span className="text-[10px] bg-gray-100 text-gray-600 border border-gray-200 px-2 py-0.5 rounded font-mono font-bold uppercase">{row.product.drug_code || '-'}</span>
                              ...
                              <div className="p-1.5 text-emerald-400 group-hover/item:text-emerald-600 group-hover/item:bg-white rounded-full transition-all shrink-0"><Search size={16} /></div>
                            </div>
                            <button
                              type="button"
                              onClick={() => handleStartScan(row.id, 'PRODUCT')}
                      ...
                      <td className="py-4 px-4 text-right">
                        <div className="flex items-center gap-1.5 justify-end">
                          <input
                            ...
                            className={`w-20 px-2.5 py-2.5 bg-white/70 backdrop-blur-sm border rounded-xl outline-none transition-all text-sm font-extrabold text-center shadow-sm
                              ${!row.product ? 'border-gray-100 text-gray-300' : row.previewError ? 'border-red-400 bg-red-50 text-red-900 focus:ring-4 focus:ring-red-100' : 'border-emerald-100 text-emerald-700 focus:ring-4 focus:ring-emerald-100 focus:border-emerald-500'}
                            `}
                          />
                          {row.product && (
                      ...
                      <td className="py-4 px-4 font-medium text-gray-700 text-center relative overflow-visible">
                        {!row.product ? <span className="text-gray-300 font-bold text-xs italic">-</span> : (
                          <div className="flex gap-2 relative overflow-visible">
                            ...
                            />
                            <button type="button" onClick={() => handleStartScan(row.id, 'LOT')} className="p-2 text-gray-400 hover:text-emerald-600 hover:bg-emerald-50 border border-gray-200 rounded-xl transition-all shrink-0" title="สแกนบาร์โค้ด Lot">
                              <Camera size={18} />
                            </button>
                          </div>
                        )}
                      ...
                      <td className="py-4 px-4 text-center text-sm font-bold">
                        {row.product && row.selected_lot ? (() => {
                          ...
                          if (lot && lot.expiry_date) {
                            ...
                            return (
                              <span className={isNearExpiry ? "text-amber-600 bg-amber-50 px-2 py-1 rounded-lg" : "text-emerald-700 bg-emerald-50 px-2 py-1 rounded-lg"}>
                      ...
                      <td className="py-4 px-4 text-center">
                        <input
                          ...
                          disabled={!row.product || isSubmitting}
                          className="w-full text-sm px-3 py-2 bg-gray-50/50 border border-gray-200 rounded-xl focus:ring-4 focus:ring-emerald-100 outline-none"
                        />
                      </td>
                      <td className="py-4 px-4 text-center">
                    ...
                    {showSplitWarning && (
                      <tr className="bg-amber-50/40">
                        <td colSpan={7} className="py-2.5 px-4 border-b border-gray-100">
                          <div className="flex items-center justify-between text-xs text-amber-800 font-bold bg-amber-50 border border-amber-100 rounded-xl px-4 py-2.5 shadow-sm">
                            <div className="flex items-center gap-2">
                              <AlertTriangle size={14} className="text-amber-500 shrink-0 animate-bounce" />
                              <span>ล็อต {row.selected_lot} มีสต๊อกไม่พอจ่าย (มี {row.availableBalances.find(b => b.lot_number === row.selected_lot)?.current_qty} ยอดสั่ง {row.qty}) แต่ยอดรวมคงคลังทุกล็อคมีเพียงพอ ({row.totalStock} {row.unit_name})</span>
                            </div>
                            <button 
                              type="button" 
                              onClick={() => handleAutoSplitRow(row.id)}
                              className="px-2.5 py-1 bg-amber-600 hover:bg-amber-700 text-white rounded-lg transition-colors text-[10px] font-black uppercase tracking-wider shrink-0 cursor-pointer shadow"
                            >
